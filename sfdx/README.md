...
- `classes/*Test.cls` — Apex tests. `OrderStatusSkillTest` seeds + exercises the Skill **inside `System.runAs` a
  ...
  the deploying user can't insert/query those fields on a fresh org (KNOWN-GAPS T12). Both classes run for the 75% gate.
...
## Deploy + seed
1. **One command (recommended):** `../scripts/06-org-onboard.sh --org <alias>` — guards Agentforce-enabled, then runs 02 → 03 → 05 → smoke. (Prereq: enable Agentforce on the org first — see [../PARTICIPANT-SETUP.md](../PARTICIPANT-SETUP.md).) Or run the steps by hand: `../scripts/02-deploy.sh --org <alias>` → `../scripts/03-assign-perms.sh --org <alias>` → `../scripts/05-seed-hero-data.sh --org <alias>` (02 creates `Order__c` but does **not** seed data — 05 is a separate required step).
